...
/**** czj modifications:
   ...
   sorting is on decreasing arity and secondary increasing name 
   fset[i].cset[j].index is set to j, not used by lilgp used by cgp
*******/
	  ...
	  /* now sort the function set so that all the functions
	     come first. */
          ...
          for (j=0; j<fset[i].size; j++)                     /* czj: bug fix */
            fset[i].cset[j].index=j;                 /* czj: set the indexes */
 ...
 * all terminals.
 */
/* czj modified to sort on # arguments nit just having arguments, and name secondary key */
int function_compare ( const void *a, const void *b )
{
     int a_ar, b_ar;
     a_ar = (((function *)a)->arity);
     b_ar = (((function *)b)->arity);
     if (a_ar>b_ar)
       return -1;
     else if (a_ar<b_ar)
       return 1;
     else return strcmp(((function *)a)->string,((function *)b)->string);
}
