...
/* czj changes: 6/10/03
   added regrow_population function, a copy of change_population
   except it always picks the last operator assumed to be regrow
*/
...
operator operator_table[] =
{
  ...
  { "uniform_mutate", operator_uniform_mutate_init },     /* czj */
  { "regrow",         operator_regrow_init },             /* czj */
  { NULL, NULL }
};
...
population *regrow_population_czj ( population *oldpop, breedphase *bp )
{
  population *newpop;
  int i, j;
  int numphases;
  ...
  while ( newpop->next < newpop->size )
  {
    i=numphases;
#ifdef DEBUG
    fprintf ( stderr, "picked operator %d\n", i );
#endif
