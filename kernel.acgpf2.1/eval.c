...
/* czj modified 6/14/04 to set the expressed_czj node field for each
     visited node in evaluation (incremented on every visit)        */
...
DATATYPE evaluate_tree_recurse ( lnode **l, int whichtree )
{
  ...
  /* step the traversal pointer forward, now that we've saved which function we're at. */
  ++*l;
