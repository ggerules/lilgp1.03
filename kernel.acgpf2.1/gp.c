...
/* czj changes to do acgp, grep on czj to see changes
   - population regrown if adjusting and regrowing, after weight change
   - generation loop modified to continue even if user criteria met
     if global Acgp_stop_on_term==0
   - adjusting/regrowing info accessed through Acgp_adj_regrow global var
   - evaluate_pop() modified to call acgp_reset_expressed_czj()
   - one bug fixed                                                        */
              ...
              event *t_eval, event *t_breed, int startfromcheckpoint )
{
  ...
  int stt_interval;
  int bestn;
  int tr = 0; //gwgtreedepth
  int genramp_max_tree_size = 17; //gwgtreedepth
  int genramp_interval = 25; //gwgtreedepth
  int use_genramp = -1; //gwgtreedepth

  //gwgtreedepth
  param = get_parameter ( "app.use_genramp" ); 
  if ( param == NULL )
  {
    /* genramp disabled. */
    use_genramp = -1;
  ...
  {
    use_genramp = atoi( param );
    if(use_genramp < 0)
      error ( E_WARNING, "\"app.use_genramp\" must be greater than or equal to 0.  defaulting to 1." );

    param = get_parameter ( "app.genramp_max_tree_depth" ); 
    if ( param == NULL )
      error ( E_FATAL_ERROR, "no value specified for \"app.genramp_max_tree_depth\"." );
    else
     genramp_max_tree_size = atoi ( param );

    param = get_parameter ( "app.genram_interval" ); 
    if ( param == NULL )
      error ( E_FATAL_ERROR, "no value specified for \"app.genram_interval\"." );
    else
      genramp_interval = atoi ( param );
  }
  ...
  /* the big loop. */
  /******** czj removed loop header begin *************************************
  for ( gen = startgen; gen <= maxgen && !term; ++gen )
  ********* czj removed loop header end, new loop header below ****************/
  for ( gen = startgen; gen <= maxgen && (!term || !Acgp_stop_on_term); ++gen )
  /******** czj ne wloop header end *******************************************/
  {
    ...
    /******* czj removed code begin ************************************************
    if ( gen != maxgen && !term )
    ******** czj removed code end, new code begin **********************************/
    if ( gen != maxgen && (!term || !Acgp_stop_on_term))
      /******* czj new code end ******************************************************/
    {
      ...
      ********** czj remove code end, start new code ****************************************/
      if (Acgp_adj_regrow<2)       /* not regrowing in this generation */
      {
        oprintf (OUT_SYS, 10, "change population...\n");
        for ( i = 0; i < mpop->size; ++i )
          mpop->pop[i] = change_population ( mpop->pop[i], mpop->bpt[i] );
      }
      else     /* regrowing in this generation */
      {
        oprintf (OUT_SYS, 10, "regrow population...\n");
        for ( i = 0; i < mpop->size; ++i )
          mpop->pop[i] = regrow_population_czj ( mpop->pop[i], mpop->bpt[i] );
      }
      /************************************************* czj new code end *******************/
      ...
      printf("use_genramp=%d\n",use_genramp );
      if(use_genramp >= 0)
      {
        //gwgtreedepth
        for(tr = 0; tr < tree_count; tr++)
        {
          printf("%s: maxdepth=%d\n", tree_map[tr].name, tree_map[tr].depthlimit);
        }
        //gwgtst code to see if I can change depthlimit half way through a run
        if((gen>startgen) && (gen % genramp_interval) == 0)
        {
          for(tr = 0; tr < tree_count; tr++)
          {
            if(tree_map[tr].depthlimit < genramp_max_tree_size) 
              tree_map[tr].depthlimit += 1 ;
          }
        }
      }
    }
                             ...
                             int bestn )
{
  ...
  acgp_adjustWeights(gen_stats, mpop->pop, mpop->size, gen,
                         atoi(get_parameter ( "max_generations" )),
                         get_parameter( "output.basename" ));
...
void evaluate_pop ( population *pop )
{
  ...
  exit(0);
#endif
  //    for(int tr = 0; tr < tree_count; tr++)
  {
    for ( k = 0; k < pop->size; ++k )
    { if ( pop->ind[k].evald != EVAL_CACHE_VALID )
      { acgp_reset_expressed_czj((pop->ind)+k);
  ...
  while ( shp )
  {
    if ( shp->refcount == 0 )
    {
      ...
      if ( saved_tail == shp )
        saved_tail = shm;
      FREE ( shp );
      shp = shm->next;        /* czj 3/12/04 changed shp->next */
 ...
 */
/*
 gwg changed fscanf %d to %ld to reflect int to long change in popstats structure
*/
...
void read_stats_checkpoint ( multipop *mpop, ephem_const **eind, FILE *f )
{
  ...
  for ( i = 0; i < mpop->size+1; ++i )
  {
    ...
    fscanf ( f, "%ld  %ld %ld %ld %ld %ld  %ld %ld %ld %ld %ld  %ld %ld %ld %ld %ld\n",
             &(run_stats[i].size),
 ...
 */
/*
 gwg changed fprintf %d to %ld to reflect int to long change in popstats structure
*/
