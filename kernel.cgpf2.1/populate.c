...
/******* czj changes:
    Modified function generate_random_population
      set Function/Argument/Type_czj to start at the Root
      use new lilgp parameter init.depth_abs
        when false, no change
        when true, use the minimal depth ramp given by init.depth
          as the absolute minimal tree depth - rejecting smaller trees
          and also setting MinDepth so that generate_random_tree_grow
          can grow at least this deep if possible
*****************/

#include <lilgp.h>

int ggmaxnegdepth = 100000;
...
void generate_random_population ( population *p, int *mindepth, int *maxdepth, int *method )
{
  int i, j, k, m;
  int attempts;
  int totalattempts = 0;
  ...
  param_czj=get_parameter("init.depth_abs");
  if (param_czj!=NULL)
    absDepth_czj=(translate_binary(param_czj)==1);
  ...
  if ( attempts_generation <= 0 )
    error ( E_FATAL_ERROR, "\"init.random_attempts\" must be positive." );
  ...
  { /* total nodes in the individual being generated. */
    ...
    { if ( attempts <= 0 )
        error ( E_FATAL_ERROR, "The last %d trees generated have been bad.  Giving up.", attempts_generation );
      ...
      if (absDepth_czj)
        MinDepth_czj=depth-mindepth[j];
      Function_czj = fset[j].function_count;
      Argument_czj = 0;     /* set to start with Root's constraints */
      Type_czj=TP_czj[j][fset[j].function_count+fset[j].terminal_count].retType;

      switch ( method[j] )
      {
      case GENERATE_FULL:
        //printf("%s\n", "gg Start Generate Random Full"); // gg
#ifdef GDEBUG
        printf("%s", "("); // gg
#endif
        generate_random_full_tree(0,depth,j,fset+tree_map[j].fset);
#ifdef GDEBUG
        printf("%s\n", ")"); // gg
#endif
        break;
      case GENERATE_GROW:
        //printf("%s\n", "gg Start Generate Random Grow");
#ifdef GDEBUG
        printf("%s", "("); // gg
#endif
        generate_random_grow_tree (0,depth,j,fset+tree_map[j].fset);
#ifdef GDEBUG
        printf("%s\n", ")"); // gg
#endif
        break;
      case GENERATE_HALF_AND_HALF:
        //printf("%s\n", "gg Start Half and Half");
      ...
      if (absDepth_czj && tree_depth(gensp[0].data)<mindepth[j])
      {
        --j;                                                 /* czj */
        continue;                     /* czj: reject too small tree */
      }
...
#if VERIFY_POPULATE_czj
    for ( j = 0; j < tree_count; ++j )
    {
      if (verify_tree_czj (j, p->ind[k].tr[j].data))
      {
        oprintf (OUT_SYS, 10, "INVALID TREE initialized\n");
        fprintf(stderr,"INVALID TREE initalized\n");
        exit(1); // causes big memory leak
      }
    }
#endif
    attempts = attempts_generation;
...
multipop *initial_multi_population ( void )
{
  ...
  if ( mpop->size <= 0 )
    error ( E_FATAL_ERROR, "\"%s\" is not a valid value for \"multiple.subpops\".", param );
  ...
  for ( i = 0; i < tree_count; ++i )
  {
    ...
    if ( param == NULL )
      error ( E_FATAL_ERROR, "\"init.tree[%d].method\" must be set.", i );
...
population *initial_population ( int *mindepth, int *maxdepth, int *method )
{
  ...
  if ( param == NULL )
    error ( E_FATAL_ERROR, "no value specified for \"pop_size\"." );
  pop_size = atoi ( param );
