...
void run_gp ( multipop *mpop, int startgen, event *t_eval, event *t_breed, int startfromcheckpoint )
{
  char *param;
  int gen;
  int maxgen;
  ...
  for ( gen = startgen; gen <= maxgen && !term; ++gen )
  {
    ...
    if ( ! ( startfromcheckpoint && gen == startgen ) )
    {
      ...
      term = generation_information ( gen, mpop, stt_interval, run_stats[0].bestn );
      if ( term )
