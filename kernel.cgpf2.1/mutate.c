...
void operator_mutate ( population *oldpop, population *newpop, void *data )
{
  ...
  while(1)
  {
    ...
    if ( forceany )
    {
      /* choose any point. */
      l = random_int ( ps );
      replace[0] = get_subtree (t, oldpop->ind[p].tr[t].data, l );
    }
    else if ( total*random_double() < md->internal )
    {
      /* choose an internal point. */
      l = random_int ( tree_nodes_internal ( oldpop->ind[p].tr[t].data ) );
      replace[0] = get_subtree_internal (t, oldpop->ind[p].tr[t].data, l );
    }
    else
    {
      /* choose an external point. */
      l = random_int ( tree_nodes_external ( oldpop->ind[p].tr[t].data ) );
      replace[0] = get_subtree_external (t, oldpop->ind[p].tr[t].data, l );
    }

#ifdef DEBUG_MUTATE
    fprintf(stdout,"subtree number: %d\n",l);         /* added by czj */
    fprintf ( stdout, "selected for replacement: " );
    ...
    {
    case GENERATE_GROW:
      generate_random_grow_tree ( 1, depth, t, fset+tree_map[t].fset );
      break;
    case GENERATE_FULL:
      generate_random_full_tree ( 1, depth, t, fset+tree_map[t].fset );
      break;
    ...
    /************ begin czj changes */
    else if (md->absDepth_czj && tree_depth( gensp[1].data ) < md->mindepth)
      badtree = 1;
    /************ end czj changes */
    ...
    {
#ifdef DEBUG_MUTATE
      fprintf ( stdout, "new tree is too big; reproducing parent.\n" );
#endif
    ...
    {
#ifdef DEBUG_MUTATE
      fprintf ( stdout, "new tree is permissible.\n" );
#endif
      ...
      copy_tree_replace_many ( 0, oldpop->ind[p].tr[t].data, replace, replace+1, 1, &repcount );
      if ( repcount != 1 )
      {
        error ( E_FATAL_ERROR,
                "botched mutation:  this can't happen." );
      }
#if VERIFY_MUTATION_czj
      if (verify_tree_czj(t, oldpop->ind[p].tr[t].data))
      {
