...
/**** czj modifications:
   one extra function call to set constraints added
   compare for sorting changed so that sorting is on arity and strings
*******/
...
int function_sets_init ( function_set *user_fset, int user_fcount,
                         int *user_tree_map, char **user_tree_name, int user_tcount )
{
  int i, j, k, m, n, p;
  int errors = 0;
  function *cur;
  ...
  for ( i = 0; i < fset_count; ++i )
  {
    ...
    qsort ( fset[i].cset, fset[i].size, sizeof ( function ),
            function_compare );
    /* czj - this fixes the index for the function set which lilgp doesn't */
    /*       use, but cgp does */
    for (j=0; j<fset[i].size; j++)                     /* czj: bug fix */
      fset[i].cset[j].index=j;                         /* czj: bug fix */
  }
...
/* function_compare()
 *
 * comparison function for qsort() that sorts on arity
 * for same arity, sorts lexicographicaly
 */

int function_compare ( const void *a, const void *b )
{
  int aa, ba;
  /*     aa = !!(((function *)a)->arity);     czj - replaced with below */
  /*     ba = !!(((function *)b)->arity);     czj - replaced with below */
  /* begin czj changes */
  aa = (((function *)a)->arity);
  ba = (((function *)b)->arity);
  if (ba-aa != 0)
    return ba-aa;
  return strcmp((((function *)a)->string),(((function *)b)->string));
  /* end czj changes */
}
...
void read_tree_limits ( void )
{
  ...
  if ( param )
  {
    ...
    if ( j >= 0 )
      for ( i = 0; i < tree_count; ++i )
        if ( tree_map[i].depthlimit < 0 || tree_map[i].depthlimit > j )
...
{
  long total, free, max, mallocc, reallocc, freec;
  ...
  oprintf ( OUT_SYS, 30, "\n------- memory -------\n" );
  oprintf ( OUT_SYS, 30, "           allocated:      %ld\n", total );
  oprintf ( OUT_SYS, 30, "               freed:      %ld\n", free );
  oprintf ( OUT_SYS, 30, "           not freed:      %ld\n", total-free );
  oprintf ( OUT_SYS, 30, "       max allocated:      %ld\n", max );
  oprintf ( OUT_SYS, 30, "    malloc'ed blocks:      %ld\n", mallocc );
  oprintf ( OUT_SYS, 30, "   realloc'ed blocks:      %ld\n", reallocc );
  oprintf ( OUT_SYS, 30, "      free'ed blocks:      %ld\n", freec );
#endif
