...
void operator_uniform_mutate ( population *oldpop, population *newpop, void *data )
{
  ...
  for (j = 0; j < mutate_count; j++)
  {
    ...
    if ( forceany )
    {
      ...
      replace[0] = get_subtree (t, newpop->ind[newpop->next].tr[t].data, l );
    }
    else if ( total*random_double() < md->internal )
    {
      ...
      replace[0] = get_subtree_internal (t, newpop->ind[newpop->next].tr[t].data, l );
    }
    else
    {
      /* choose an external point. */
      l = random_int ( tree_nodes_external ( newpop->ind[newpop->next].tr[t].data ) );
      replace[0] = get_subtree_external (t, newpop->ind[newpop->next].tr[t].data, l );
    }
