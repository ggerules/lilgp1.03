...
int operator_crossover_init ( char *options, breedphase *bp )
{
  ...
  double r;
  char **argv, **targv;
  int internalset_src = 0, externalset_src = 0;
  int internalset_dst = 0, externalset_dst = 0;
  char *cp;
  ...
  for ( i = 0; i < j; ++i )
  {
    ...
    {
      internalset_src = 1;
      cd->internal_src = strtod ( argv[++i], NULL );
      cd->internal_dst = cd->internal_src;
      if ( cd->internal_src < 0.0 )
      {
        ++errors;
    ...
    {
      externalset_src = 1;
      cd->external_src = strtod ( argv[++i], NULL );
      cd->external_dst = cd->external_src;
      if ( cd->external_src < 0.0 )
      {
        ++errors;
    ...
    /* parse "internal_dst" option. */
    else if ( strcmp ( "internal_dst", argv[i] ) == 0 )
    {
      internalset_dst = 1;
      cd->internal_dst = strtod ( argv[++i], NULL );
      if ( cd->internal_dst < 0.0 )
      {
        ++errors;
        error ( E_ERROR, "crossover: \"internal_dst\" must be nonnegative." );
      }
    }
    /* parse "external_dst" option. */
    else if ( strcmp ( "external_dst", argv[i] ) == 0 )
    {
      externalset_dst = 1;
      cd->external_dst = strtod ( argv[++i], NULL );
      if ( cd->external_dst < 0.0 )
      {
        ++errors;
        error ( E_ERROR, "crossover: \"external_dst\" must be nonnegative." );
      }
    }
  ...
  if ( internalset_src && !externalset_src )
    cd->external_src = 0.0;
  else if ( !internalset_src && externalset_src )
    cd->internal_src = 0.0;

  if ( internalset_dst && !externalset_dst )
    cd->external_dst = 0.0;
  else if ( !internalset_dst && externalset_dst )
    cd->internal_dst = 0.0;
  ...
  if ( !errors )
  {
    ...
    // printf ( "   internal_src: %lf  external_src: %lf\n", cd->internal_src, cd->externali_src );
    // printf ( "   internal_dst: %lf  external_dst: %lf\n", cd->internal_dst, cd->externali_dst );
...
/* czj: and to use separate external/internal parameters for source/dest */
...
void operator_crossover ( population *oldpop, population *newpop, void *data )
{
  ...
  int ns1, ns2;
  int badtree=0;
  double total_src, total_dst;
  int forceany1_src, forceany1_dst, forceany2_src, forceany2_dst;
  int repcount;
  ...
  cd = (crossover_data *)data;
  total_src = cd->internal_src + cd->external_src;
  total_dst = cd->internal_dst + cd->external_dst;
  ...
  forceany1_src = (ps1==1||total_src==0.0);
  forceany1_dst = (ps1==1||total_dst==0.0);

  p2 = cd->sc2->select_method ( cd->sc2 );
  ps2 = oldpop->ind[p2].tr[t2].nodes;
  forceany2_src = (ps2==1||total_src==0.0);
  forceany2_dst = (ps2==1||total_dst==0.0);
  ...
  {
    if (forceany1_dst)                                        /* choose dest */
    {
      l1 = random_int(ps1);
      st[1] = get_subtree(t1, oldpop->ind[p1].tr[t1].data, l1);
    }
    else if (total_dst*random_double() < cd->internal_dst)      /* int. dest */
    {
      l1=random_int(tree_nodes_internal(oldpop->ind[p1].tr[t1].data));
      st[1] = get_subtree_internal(t1, oldpop->ind[p1].tr[t1].data, l1);
    }
    else                                     /* choose an external dest */
    {
      l1=random_int(tree_nodes_external(oldpop->ind[p1].tr[t1].data));
      st[1] = get_subtree_external(t1, oldpop->ind[p1].tr[t1].data,l1);
    }
    /* Function_/Argument_/Type_czj are set by get_subtree* */
    repBad_czj=0;                /* reset the limit counter for the new dest */
    if (markXNodes_czj(t2, oldpop->ind[p2].tr[t2].data)==0)
      continue;                        /* no sources found, try another dest */
    while (repBad_czj++<RepeatsBad_czj)  /* limit on alt. srcs for this dest */
    {
      if (forceany2_src)             /* here only if some sources were found */
        st[2]=getSubtreeMarked_czj(t2, oldpop->ind[p2].tr[t2].data,0);
      else if (total_src*random_double() < cd->internal_src)
        st[2]=getSubtreeMarked_czj(t2, oldpop->ind[p2].tr[t2].data,1);
      else
        st[2]=getSubtreeMarked_czj(t2, oldpop->ind[p2].tr[t2].data,2);
#ifdef DEBUG_CROSSOVER
    ...
    if (verify_tree_czj (t1, newpop->ind[newpop->next-1].tr[t1].data))
    {
  ...
  if ( newpop->next < newpop->size )
  {
    ...
    while (repSrc_czj++<RepeatsSrc_czj)
    {
      ...
      {
        l2 = random_int(ps2);
        st[2] = get_subtree(t2,oldpop->ind[p2].tr[t2].data,l2);
      }
      else if (total_dst*random_double() < cd->internal_dst)
      {
        l2=random_int(tree_nodes_internal(oldpop->ind[p2].tr[t2].data));
        st[2]=get_subtree_internal(t2, oldpop->ind[p2].tr[t2].data,l2);
      }
      else
      {
        l2=random_int(tree_nodes_external(oldpop->ind[p2].tr[t2].data));
        st[2]=get_subtree_external(t2, oldpop->ind[p2].tr[t2].data,l2);
      }                     /* Function_/Argument_/Type_czj will be set */
      repBad_czj=0;
      if (markXNodes_czj(t1, oldpop->ind[p1].tr[t1].data)==0)
        continue;                            /* no sources, try another dest */
      while (repBad_czj++<RepeatsBad_czj)
      {
        if (forceany1_src)
          st[1]=getSubtreeMarked_czj(t1,oldpop->ind[p1].tr[t1].data,0);
        else if (total_src*random_double() < cd->internal_src)
          st[1]=getSubtreeMarked_czj(t1,oldpop->ind[p1].tr[t1].data,1);
        else
          st[1]=getSubtreeMarked_czj(t1,oldpop->ind[p1].tr[t1].data,2);
      ...
      if (verify_tree_czj (t2, newpop->ind[newpop->next-1].tr[t2].data))
      {
