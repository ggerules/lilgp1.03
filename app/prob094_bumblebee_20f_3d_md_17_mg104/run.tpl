...
[+IF (match-value? == "kname" "kernel.acgp1.1.2")+]
  ./$exe -p pop_size=$pop -p max_generations=$maxgen -p random_seed=$1 -p app.lawn_width=$lw -p app.lawn_height=$lh -p app.lawn_depth=$ld -p app.use_ercs=$useercs -p app.save_pop=$savepop -p acgp.what=$acgpwhat -p max_depth=$maxdepth -p output.basename=tmp/$prblm-$frmwrk-$1-$wadf-$adf-$types-$constrained-$acgpwhat-$maxdepth -f input.file < acgp1p1p2_cli_input.txt
[+ELSE+]
