...
int app_end_of_evaluation ( int gen, multipop *mpop, int newbest, popstats *gen_stats, popstats *run_stats )
{
  ...
  for ( i = 0; i < run_stats[0].bestn; ++i )
  {
    ...
    for(; tr >= 0; tr--) // tree
    {
      ...
      { 
        [+ FOR nadf+][+FOR fset+][+ FOR vars +]fprintf(fptrbind, "(de %s ([+altname+]) (declare (-double-) x)", tree_map[tr].name);[+ ENDFOR +][+ENDFOR+][+ENDFOR+]
        pretty_print_tree ( run_stats[0].best[i]->ind->tr[tr].data, fptrbind);
      }
      fprintf(fptrbind, "%s", ")");
      fprintf(fptrbind, "%s", "\n");
...
int app_end_of_evaluation ( int gen, multipop *mpop, int newbest, popstats *gen_stats, popstats *run_stats )
{
  ...
  for ( i = 0; i < run_stats[0].bestn; ++i )
  {
    ...
    for(; tr >= 0; tr--) // tree
    {
      ...
      {
        [+ FOR yadf+][+FOR fset+][+ FOR vars +]fprintf(fptrbind, "(de %s ([+altname+]) (declare (-double-) x)", tree_map[tr].name);[+ ENDFOR +][+ENDFOR+][+ENDFOR+]
        pretty_print_tree ( run_stats[0].best[i]->ind->tr[tr].data, fptrbind);
      }
      fprintf(fptrbind, "%s", ")");
      fprintf(fptrbind, "%s", "\n");
