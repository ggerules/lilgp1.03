...
# cgp2p1p_simple_trig_no_adf

# -----------
cgp2p1_no_constraints <- read.table("/home/ggerules/lilgp1.02/app/simple_trig/data/cgp2p1_simple_trig_no_adf/no_constraints/tmp/hits.txt", header=FALSE, sep=",", na.strings="NA", dec=".", strip.white=TRUE)
...
printf("%s", "cgp2p1_simple_trig_no_adf_no_constraints: " )
...
cgp2p1_constraints <- read.table("/home/ggerules/lilgp1.02/app/simple_trig/data/cgp2p1_simple_trig_no_adf/constraints/tmp/hits.txt", header=FALSE, sep=",", na.strings="NA", dec=".", strip.white=TRUE)

cgp2p1_constraints <- with(cgp2p1_constraints, cgp2p1_constraints[order(V3, decreasing=FALSE), ])

#hist(x=orignoadf$V9, freq=TRUE)

printf("%s", "cgp2p1_simple_trig_no_adf_constraints:    " )
