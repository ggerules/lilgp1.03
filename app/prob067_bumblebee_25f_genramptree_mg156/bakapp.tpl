...
int app_end_of_evaluation ( int gen, multipop *mpop, int newbest, popstats *gen_stats, popstats *run_stats )
{
  ...
  [+ENDFOR func+][+ENDFOR lispdefun+][+FOR adffunc+][+FOR lispdefun+]fprintf(fptrbind, "%s\n", "[+row+]");
  [+ENDFOR lispdefun+][+ENDFOR func+][+ENDIF+][+ENDFOR fset+][+ENDFOR yadf+]
  [+ FOR yadf+][+FOR fset +][+IF (not (first-for? "fset"))+]fprintf(fptrbind, "%s", "[+FOR adfarg +] [+altname+][+ENDFOR adfarg+]");[+ENDIF+][+ENDFOR fset+][+ENDFOR yadf+] 
  fprintf(fptrbind, "%s", ") ");
  [+ FOR yadf+][+FOR fset +][+IF (not (first-for? "fset"))+][+FOR adfarg +]fprintf(fptrbind, " (declare (-double-) %s)", "[+altname+]");[+ENDFOR adfarg+][+ENDIF+][+ENDFOR fset+][+ENDFOR yadf+] 
  for ( i = 0; i < run_stats[0].bestn; ++i )
  {
    //pretty_print_individual ( run_stats[0].best[i]->ind, fptrbind);
    int tr = tree_count-1;
