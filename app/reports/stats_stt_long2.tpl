...
dat <- read.table("/home/ggerules/lilgp1.02/app/data/sttlong.csv", header=TRUE, sep=",", na.strings="NA", dec=".", strip.white=TRUE)
...
#
#plot <- ggplot(df, aes(x=x, y=val)) + geom_line(color="red") +
#  labs(title="[+prob.name+] - Mean Tree Size Worst of Run Individuals vs Generation Number\n by Generation Number", subtitle="Original kernel.orig Used\n 30 Independent Runs\n (Kernel == [+frmwrk+], wADF == [+wadf+], ADF == [+adf+], Types == [+types+], Constraints == [+cons+], acgpwhat == [+what+]) ", x="Generation Number", y="Mean Tree Size Worst of Run Individuals") + ylim(0,100)
#ggsave(plot, file="./plots/kgt_[+dirname+].pdf")
#
#x <- c(o1$GenNum) 
#str(x)
#framework <- c(rep("[+dirname+]",51))
...
#
#plot <- ggplot(df, aes(x=x, y=val)) + geom_line(color="red") +
#  labs(title="[+prob.name+] - Mean Tree Depth Worst of Run Individual vs Generation Number\n by Generation Number", subtitle="Original kernel.orig Used\n 30 Independent Runs\n (Kernel == [+frmwrk+], wADF == [+wadf+], ADF == [+adf+], Types == [+types+], Constraints == [+cons+], acgpwhat == [+what+]) ", x="Generation Number", y="Mean Tree Depth Worst of Run Individuals") + ylim(0,10)
#ggsave(plot, file="./plots/lgt_[+dirname+].pdf")
