 ...
 *  
 */
 /* this code was modified on 21 July 2010 to solve the equation
 *   (x * x) + (y * y) + (z * z)
 */
...
static double *app_fitness_cases[4];
 ...
 */
     ...
     { { f_multiply,   NULL,      NULL,         2, "mul",     FUNC_DATA, -1, 0 },
       ...
       { f_indepvar_x,   NULL,      NULL,         0, "X",     TERM_NORM, -1, 0 },
       { f_indepvar_y,   NULL,      NULL,         0, "Y",     TERM_NORM, -1, 0 },
       { f_indepvar_z,   NULL,      NULL,         0, "Z",     TERM_NORM, -1, 0 },
       { f_const_c_5,   NULL,      NULL,         0, "C_5",     TERM_NORM, -1, 0 },
       { f_const_c_4,   NULL,      NULL,         0, "C_4",     TERM_NORM, -1, 0 },
       { f_const_c_3,   NULL,      NULL,         0, "C_3",     TERM_NORM, -1, 0 },
       { f_const_c_2,   NULL,      NULL,         0, "C_2",     TERM_NORM, -1, 0 },
       { f_const_c_1,   NULL,      NULL,         0, "C_1",     TERM_NORM, -1, 0 },
       { f_const_c0,   NULL,      NULL,         0, "C0",     TERM_NORM, -1, 0 },
       { f_const_c1,   NULL,      NULL,         0, "C1",     TERM_NORM, -1, 0 },
       { f_const_c2,   NULL,      NULL,         0, "C2",     TERM_NORM, -1, 0 },
       { f_const_c3,   NULL,      NULL,         0, "C3",     TERM_NORM, -1, 0 },
       { f_const_c4,   NULL,      NULL,         0, "C4",     TERM_NORM, -1, 0 },
       { f_const_c5,   NULL,      NULL,         0, "C5",     TERM_NORM, -1, 0 } };
       //{ NULL,         f_erc_gen, f_erc_print,  0, "R",     TERM_ERC,  -1, 0 } };
...
void app_eval_fitness ( individual *ind )
{
     ...
     for ( i = 0; i < fitness_cases; ++i )
     {
          ...
          g.y = app_fitness_cases[1][i];
          g.z = app_fitness_cases[2][i];
          v = evaluate_tree ( ind->tr[0].data, 0 );
          dv = app_fitness_cases[3][i];
          disp = fabs ( dv-v );
          //disp = ( dv-v )*( dv-v );
                           ...
                           popstats *gen_stats, popstats *run_stats )
{
     ...
     if ( newbest )
     {
          ...
          for ( i = -100; i <= 100; ++i )
          {
               ...
               oprintf ( OUT_USER, 50, "%lf %lf %lf %lf\n", g.x, g.y, g.z, v );
          }
...
int app_initialize ( int startfromcheckpoint )
{
     ...
     if ( !startfromcheckpoint )
     {
	  ...
	  app_fitness_cases[1] = (double *)MALLOC ( fitness_cases *
						   sizeof ( double ) );
      app_fitness_cases[2] = (double *)MALLOC ( fitness_cases *
						   sizeof ( double ) );
      app_fitness_cases[3] = (double *)MALLOC ( fitness_cases *
						   sizeof ( double ) );
	  
	  oprintf ( OUT_PRG, 100, "%d fitness cases:\n", fitness_cases );
	  for ( i = 0; i < fitness_cases; ++i )
	  {
	       x = (random_double()*20.0)-10.0;
	       y = (random_double()*20.0)-10.0;
	       z = (random_double()*20.0)-10.0;
	       
	       /* change this line to modify the goal function. */
	       //y = x*x*x*x + x*x*x + x*x + x;
	       /*the new bowl3 goal function */
	       v = (x * x) + (y * y) + (z * z);
	       
	       app_fitness_cases[0][i] = x;
	       app_fitness_cases[1][i] = y;
	       app_fitness_cases[2][i] = z;
	       app_fitness_cases[3][i] = v;
     
	       oprintf ( OUT_PRG, 50,
			"    x = %12.5lf, y = %12.5lf, z = %12.5lf, v = %12.5lf\n",
             x, y, z, v );
	  }
     }
     else
...
void app_read_checkpoint ( FILE *f )
{
     ...
     for ( i = 0; i < fitness_cases; ++i )
     {
	  ...
	  fprintf ( stderr, "%.5lf %.5lf %.5lf\n", app_fitness_cases[0][i],
		   app_fitness_cases[1][i], app_fitness_cases[2][i] );
     }
}
