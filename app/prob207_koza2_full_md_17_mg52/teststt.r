...
dat <- read.table("/home/ggerules/lilgp1.02/app/prob004_koza1/data/sttlong.csv", header=TRUE, sep=",", na.strings="NA", dec=".", strip.white=TRUE)
...
#plot <- ggplot(va) + labs(title="Koza2 - Mean Standard Fitness of Run vs Generation Number", subtitle="for kernel orig_ywadf_nadf_ntypes_ncons_nwhatn", x="Generation", y="Mean Standard Fitness of Run") + geom_point(aes(x=d$GenNum, y=d$MeanStdFitnessOfRun, color=d$IndRunNum)) + geom_smooth(aes(x=d$GenNum, y=d$MeanStdFitnessOfRun)) + scale_color_continuous(name="Independant Run Num")

#plot <- ggplot(dd_1) + labs(title="Koza2 - Mean Standard Fitness Best Of Run Individual", subtitle="for kernel orig_ywadf_nadf_ntypes_ncons_nwhatn", x="Generation", y="Mean Standard Fitness Best Of Run Individual") + geom_line(aes(x=dd_1$GenNum, y=dd_1$MeanStdFitnessBestOfRunInd)) 
plot <- ggplot(dd,aes(x=dd$GenNum, y=dd$MeanStdFitnessBestOfRunInd, color=dd$ADF)) + labs(title="Koza2 - Mean Standard Fitness of Run vs Generation Number", subtitle="for kernel orig_ywadf_nadf_ntypes_ncons_nwhatn", x="Generation", y="Mean Standard Fitness of Run") + geom_line() + guides(fill = guide_legend(title="Independant Run Num"))
