...
{ collapse_data * cd;
  ...
  {                                                  /* choose internal dest */
    ...
    st[1] = get_subtree_internal(oldpop->ind[p].tr[t].data, l); /* true tree */
    st[0] = get_subtree_internal(newtree_p->data, l);           /* copy of tree */
#ifdef DEBUG_COLLAPSE
    printf("Trying dest %d\n",l);
#endif
   /* czj: note that Function_/Argument_czj are set by calls to get_subtree* */
    if (markXNodesNoRoot_czj( st[0] )==0)
      continue;                        /* no sources found, try another dest */
    if (forceany)                  /* here only if some sources were found */
      st[2]=getSubtreeMarked_czj( st[0] ,0);
    else if (total*random_double() < cd->internal)
           st[2]=getSubtreeMarked_czj( st[0] ,1);
         else 
           st[2]=getSubtreeMarked_czj( st[0],2);
    sourcesFound = 1;
#ifdef DEBUG_COLLAPSE
  ...
  if (verify_tree_czj ( newpop->ind[newpop->next-1].tr[t].data))
