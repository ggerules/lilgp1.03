...
// gwgnote: put parenthsis around individual for debugging purposes
#define GDEBUG 0
...
{    int i, j;
     ...
     { //fprintf(stderr,"cannot instantiate a tree\n");
       ...
       oprintf ( OUT_SYS, 20, "gg max neg tree depth %d, called from generate_random_full_tree \n", ggmaxnegdepth );
       exit(1);
     }
#endif

     if (depth < 0) // gwg it was originally this, this violates maxdepth constraints as set in input.file!!!!
       i=random_T_czj();
     else
       i=random_F_czj();          /* try function, but not guaranteed either */
     ...
     rand_czj = random_int(TP_czj[i].f.indexes[Type_czj].len);
     index_czj = TP_czj[i].f.indexes[Type_czj].indexes[rand_czj];
     ...
     { case FUNC_DATA: case EVAL_DATA:
         ...
         { Function_czj = i;
           Argument_czj = j;
           Type_czj = TP_czj[i].f.typeVecs[index_czj][j];
...
{    int i, j;
     ...
     { //fprintf(stderr,"cannot instantiate a tree\n");
       fprintf(stdout,"\ncannot instantiate a tree\n");
       //gwg 
       oprintf ( OUT_SYS, 20, "gg max neg tree depth %d, called from generate_random_grow_tree \n", ggmaxnegdepth );
       exit(1);
     }
#endif
...
lnode *get_subtree_recurse ( lnode **l, int *c )
{
     ...
     else
     {
		 ...
		 Argument_czj = i;
                 Type_czj = TP_czj[Function_czj].f.typeVecs[vector_czj][i];
